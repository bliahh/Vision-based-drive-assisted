    ...
    const std::vector<float>& vertices, float r, float g, float b, float alpha)
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(program_lane, "uMVP"), 1, GL_FALSE, glm::value_ptr(mvp));
    glUniform1f(glGetUniformLocation(program_lane, "uTime"), elapsed_time);
    glUniform4f(glGetUniformLocation(program_lane, "uColor"), r, g, b, alpha);
    glUniform1i(glGetUniformLocation(program_lane, "uFogEnabled"), fog_enabled ? 1 : 0);
    glUniform1f(glGetUniformLocation(program_lane, "uFogDensity"), fog_density);
