...
void HudRenderer::drawStars() {
    ...
    GLint loc_mvp = glGetUniformLocation(program_stars, "uMVP");
    GLint loc_color = glGetUniformLocation(program_stars, "uColor");
    GLint loc_time = glGetUniformLocation(program_stars, "uTime");

    glm::mat4 mvp = buildMVP(viewport_width, viewport_height);

    glUniformMatrix4fv(loc_mvp, 1, GL_FALSE, glm::value_ptr(mvp));
    glUniform4f(loc_color, 1.0f, 1.0f, 0.9f, 1.0f);
    glUniform1f(loc_time, (float)elapsed_time);
