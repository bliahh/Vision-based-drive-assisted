...
void HudRenderer::drawEgoCorridor(const PolyVec& lane_lines, const PolyVec& drivable_area) {
    ...
    for (int lane = 0; lane < nr_lanes; lane++) {
        ...
        if (is_ego_lane) {
            drawTriangleStrip(vao_lane, vbo_lane, buildTriangleStrip(left_edge, right_edge, 0.15f), 0.f, 1.f, 0.53f, 1.f);
            drawLines(vao_lane, vbo_lane, buildLineVertices(left_edge, 0.9f), 1.f, 1.f, 1.f, 0.85f, 3.f, GL_LINE_STRIP);
            drawLines(vao_lane, vbo_lane, buildLineVertices(right_edge, 0.9f), 1.f, 1.f, 1.f, 0.85f, 3.f, GL_LINE_STRIP);
        } else {
            drawLines(vao_lane, vbo_lane, buildLineVertices(left_edge, 0.7f), 1.f, 1.f, 1.f, 0.65f, 2.5f, GL_LINE_STRIP);
            drawLines(vao_lane, vbo_lane, buildLineVertices(right_edge, 0.7f), 1.f, 1.f, 1.f, 0.65f, 2.5f, GL_LINE_STRIP);
        }
    }
}
